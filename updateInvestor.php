...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE investor SET InvestorName = ?, PhoneNumber = ?, Email = ?, Address = ?, 
    ...
    $stmt->bind_param("sssssssi", $InvestorName, $PhoneNumber, $Email, $Address, 
                      $aadharCardPath, $panCardPath, $domicilePath, $investorId);

    if ($stmt->execute()) {
        echo "<p class='success'>Investor details updated successfully.</p>";
    } else {
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT investorId, InvestorName, PhoneNumber, Email, Address, AadharCard, PanCard, Domicile FROM investor WHERE investorId = ?";
    $stmt = $conn->prepare($sql);
...
<body>
    <div class="container">
        <h2>Edit Investor Details</h2>

        <form action="" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="investorId" value="<?php echo $row['investorId']; ?>">

            <label for="InvestorName">Name:</label>
            <input type="text" name="InvestorName" value="<?php echo htmlspecialchars($row['InvestorName']); ?>" required>
